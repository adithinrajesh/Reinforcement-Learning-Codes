...
ENVIRONMENTS = {
    "FrozenLake-v1": {
        ...
        "policies": {
            ...
            "improved_heuristic_policy": None,  # Will define this later
        },
    },
    "MountainCar-v0": {
        "render_mode": "human",
...
# Define the improved heuristic policy
def improved_heuristic_policy(state):
    """
    This heuristic policy prioritizes moving right until the last column,
    then moves down to reach the goal (G). It tries to avoid holes.
    """
    if state == 0:  # S
        return 1  # Down
    elif state == 1:  # Safe
        ...
        return 2  # Right
    elif state == 3:  # Rightmost column, need to go down
        return 1  # Down
    elif state == 4:  # Safe
        return 1  # Down
    elif state == 5:  # Hole
        ...
        return 2  # Right
    elif state == 14:  # Safe, near the goal
        return 2  # Right
    else:  # Goal state (15)
        return 2  # Stay in place

# Update the improved heuristic policy in the ENVIRONMENTS dictionary
ENVIRONMENTS["FrozenLake-v1"]["policies"]["improved_heuristic_policy"] = improved_heuristic_policy
...
policy_name = "improved_heuristic_policy"  # Change to another policy name as needed
