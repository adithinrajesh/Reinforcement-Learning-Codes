...
ENVIRONMENTS = {
    "FrozenLake-v1": {
        ...
        "policies": {
            ...
            "heuristic_policy": None,  # Will define this later
        },
    },
    "MountainCar-v0": {
        "render_mode": "human",
...
def heuristic_policy(state):
    """
    This heuristic policy prioritizes moving right or down to reach the goal (G).
    It avoids holes and aims to reach the terminal state efficiently.
    """
    if state in [0, 1, 2, 4, 5, 6, 8, 9, 10]:  # Safe to move right
        return 2  # Move right
    elif state in [3, 7, 11]:  # Edge states in the right column, move down
        return 1  # Move down
    elif state in [12, 13, 14]:  # Bottom row, move right towards the goal
        return 2  # Move right
    else:
        return 1  # Move down by default

# Update the heuristic policy in the ENVIRONMENTS dictionary
ENVIRONMENTS["FrozenLake-v1"]["policies"]["heuristic_policy"] = heuristic_policy
...
policy_name = "heuristic_policy"  # Change to another policy name as needed
